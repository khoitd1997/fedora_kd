...
function btrfs_get_partition {
    ...
    btrfs_partition=$(mount -l | grep "on ${btrfs_partition_search} type btrfs" |  awk '{ print $1 }')
    export btrfs_partition
}

function btrfs_get_partition_disk_uuid {
    btrfs_get_partition
    btrfs_partition_uuid=$(ls -l /dev/disk/by-uuid | grep "${btrfs_partition}" | awk '{ print $9 }')
    export btrfs_partition_uuid
}
