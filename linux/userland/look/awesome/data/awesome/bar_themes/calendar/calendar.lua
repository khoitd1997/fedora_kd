-- source: https://github.com/deficient/calendar/blob/master/calendar.lua
...
function calendar:init(args)
    self.num_lines   = 0
    self.today_color = beautiful.fg_focus
    self.anyday_color = beautiful.fg_normal
    ...
    self.anyday      = args.anyday      or '<b><span color="' .. self.anyday_color .. '">%2i</span></b>'
...
function calendar:page(month, year)
    ...
    for day = 1, dB.day do
        if column == 7 then
            column = 0
            nLines = nLines + 1
            page = page .. "\n" .. format_date("", {year=year, month=month, day=day})
        end
